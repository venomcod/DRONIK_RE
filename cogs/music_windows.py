"""Music cog for Windows.

Depends on system `ffmpeg` (provide path via FFMPEG_PATH env var or install ffmpeg and add to PATH).
Usage:
- Install requirements: see ../requirements.txt
- In Discord voice channel, use `!join` then `!play <youtube_url_or_search>`

This cog uses `yt-dlp` to extract audio stream URLs and plays via FFmpegPCMAudio.
"""
...
def resolve_deno_path() -> Optional[str]:
    ...
    candidate2 = Path.home() / '.deno' / 'bin' / 'deno'
...
class MusicWindows(commands.Cog):
    """Basic music player cog tuned for Windows (custom ffmpeg path supported)."""
    ...
    async def play(self, ctx: commands.Context, *, query: str):
        ...
        ffmpeg_path = os.getenv('FFMPEG_PATH_WINDOWS') or os.getenv('FFMPEG_PATH')
        try:
...
async def setup(bot: commands.Bot):
    await bot.add_cog(MusicWindows(bot))
